...
CREATE OR REPLACE FUNCTION list_items(IN p_list_id INTEGER, OUT id INTEGER, OUT name VARCHAR(255), OUT status INTEGER) RETURNS setof RECORD AS $$
        SELECT l.id, name, status
        FROM item i
